...
# Agora lê do CSV público recém-gerado
CSV_MASTER_PATH = os.path.join(RAIZ_PROJETO, 'auditoria/dataset_multitask_unificado_public.csv')

# Pastas de saída dos splits com os nomes semânticos das tarefas
RAIZ_ORIENTATION = os.path.join(RAIZ_PROJETO, 'splits/Orientation')
RAIZ_SUITABILITY = os.path.join(RAIZ_PROJETO, 'splits/VMMRSuitability')
RAIZ_COLOR = os.path.join(RAIZ_PROJETO, 'splits/ColorClarity')

def salvar_txt_split(df_particao, coluna_label, caminho_arquivo):
    os.makedirs(os.path.dirname(caminho_arquivo), exist_ok=True)
    with open(caminho_arquivo, "w") as f:
        for _, row in df_particao.iterrows():
            img_path_rel = str(row['image_path']).strip()
            label_val = int(row[coluna_label])
            # Grava no formato limpo: UFOP/ufop_011.JPG 0
            f.write(f"{img_path_rel} {label_val}\n")
...
def processar_protocolos_para_label(df_dados, coluna_label, raiz_saida):
    ...
    # Filtros baseados na substring do caminho relativo
